...
describe.skip("Index", function () {
  ...
  const approveDeposit = async ({ token, target, depositer, amount }) => {
    ...
    if (m[target.address].type == "index") {
      ...
      u[`${depositer.address}`].balance =
        u[`${depositer.address}`].balance.sub(amount);
      u[`${depositer.address}`].deposited[`${target.address}`] =
        u[`${depositer.address}`].deposited[`${target.address}`].add(amount);
      u[`${depositer.address}`].lp[`${target.address}`] =
        u[`${depositer.address}`].lp[`${target.address}`].add(_mintAmount);
      ...
      u[`${depositer.address}`].balance =
        u[`${depositer.address}`].balance.sub(amount);
      u[`${depositer.address}`].deposited =
        u[`${depositer.address}`].deposited.add(amount);
      u[`${depositer.address}`].lp =
        u[`${depositer.address}`].lp.add(_mintAmount);

      expect(await token.balanceOf(depositer.address)).to.equal(
        u[`${depositer.address}`].balance
      );
      expect(await target.balanceOf(depositer.address)).to.equal(
        u[`${depositer.address}`].lp
      );
      ...
      if (!m1.utilizationRate.isZero()) {
        m1.utilizationRate = UTILIZATION_RATE_LENGTH_1E8.mul(m1.insured).div(
          m1.marketBalance
        );
      } else {
      ...
      await verifyValueOfUnderlying({
        ...
        valueOfUnderlying: u[`${depositer.address}`].lp
          .mul(m1.rate)
          .div(defaultRate),
      });
