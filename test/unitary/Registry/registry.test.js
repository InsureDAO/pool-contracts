...
describe("registry", function () {
  ...
  describe("setFactory() / factory()", function () {
    it("reverts when called by non-admin address", async () => {
      it("reverts", async function () {
        await expect(registry.connect(alice).setFactory(factory.address)).to.revertedWith(
    ...
    it("should not be zero address", async () => {
      it("reverts", async function () {
        await expect(registry.setFactory(ZERO_ADDRESS)).to.revertedWith("ERROR: ZERO_ADDRESS");
      });
    });
  ...
  describe("setExistence() / confirmExistence()", function () {
    it("msgSender should be either the factory address or the owner address", async () => {
      ...
      await registry.connect(factory).setExistence(market2.address, ZERO_ADDRESS);
      await expect(registry.connect(alice).setExistence(market3.address, ZERO_ADDRESS)).to.revertedWith(
        "ERROR: UNAUTHORIZED_CALLER"
      );
    });
    it("should set existence if registered", async () => {
      await registry.setExistence(market1.address, ZERO_ADDRESS);
      expect(await registry.confirmExistence(market1.address, ZERO_ADDRESS)).to.equal(true);
      expect(await registry.confirmExistence(market2.address, ZERO_ADDRESS)).to.equal(false);
    });
  });
