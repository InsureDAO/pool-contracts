...
describe('test BondingPremium', () => {
    ...
    describe("test setPremium", function () {
      it("setPremium correctly", async () => {
        ...
        let a = (ten_to_the_6.add(await sqrt(ten_to_the_6.mul(ten_to_the_6).add(k.mul(4))))).div(2).sub(ten_to_the_6);
      ...
      it("revert setPremium", async () => {
        ...
        await expect(premium.connect(alice).setPremium(b, k)).to.revertedWith("Ownable: caller is not the owner");
      });
    ...
    describe("test setOptions", function () {
      it("setOptions correctly", async () => {
        ...
        await premium.setOptions(low_risk_liquidity, low_risk_b, low_risk_util, 0);
      ...
      it("revert setOptions", async()=>{
        ...
        await expect(premium.setOptions(0, low_risk_b, 0, 0)).to.revertedWith("low_risk_base_fee must lower than base_fee");
        await expect(premium.connect(alice).setOptions(0, 0, 0, 0)).to.revertedWith("Ownable: caller is not the owner");
      });
    });
    ...
    describe("test getPremium", function () {
      ...
      it("low risk getPremium correctlly", async () => {
        ...
        //expection
        let util = locked_amount.add(amount).add(locked_amount).mul(ten_to_the_6).div(total).div(2);
        let Q = ten_to_the_6.sub(util).add(a);
        let _premiumRate = k.mul(365).sub(Q.mul(a).mul(365)).add(Q.mul(low_risk_b)).div(Q);
        let expected = amount.mul(_premiumRate).mul(length).div(YEAR).div(ten_to_the_6);
