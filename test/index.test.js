...
describe("Index", function () {
  ...
  describe("Liquidity providing life cycles", function () {
    ...
    it("DISABLES withdraw when liquidity is locked for insurance", async function () {
      ...
      expect(await index.withdrawable()).to.equal("2697");
      await ethers.provider.send("evm_increaseTime", [86400 * 8]);
      await expect(index.connect(alice).withdraw("10000")).to.revertedWith(
        "ERROR: WITHDRAWAL_BAD_CONDITIONS"
      );
    ...
    it("accrues premium after deposit", async function () {
      ...
      expect(await dai.balanceOf(bob.address)).to.closeTo("97200", "5"); //verify
      expect(await index.totalLiquidity()).to.closeTo("12697", "5");
      ...
      expect(await dai.balanceOf(alice.address)).to.closeTo("102700", "3"); //verify
    });
    ...
    it("devaluate underlying when cover claim is accepted", async function () {
      ...
      expect(await dai.balanceOf(bob.address)).to.closeTo("99940", "2");
      ...
      expect(await vault.underlyingValue(creator.address)).to.closeTo("2", "1");
      ...
      expect(await index.totalSupply()).to.equal("10000");
      expect(await index.totalLiquidity()).to.closeTo("5060", "1");

      expect(await index.totalAllocatedCredit()).to.closeTo("10120", "2");
      expect(await market2.allocatedCredit(index.address)).to.closeTo(
        "5060",
        "1"
      );
      expect(await market2.totalLiquidity()).to.closeTo("5060", "1");
      expect(await market1.totalLiquidity()).to.closeTo("5060", "1");
      expect(await vault.underlyingValue(index.address)).to.closeTo(
        "5060",
        "1"
      );
      ...
      expect(await index.totalSupply()).to.equal("10000");
      expect(await index.totalLiquidity()).to.closeTo("60", "1");
  ...
  describe("Index parameter configurations (case un-equal allocation)", function () {
    ...
    it("allows new pool addition", async function () {
      ...
      expect(await index.withdrawable()).to.equal("73");

      await index.set(market3.address, "2000");
      expect(await index.totalSupply()).to.equal("10000");
      expect(await index.totalLiquidity()).to.equal("10073");
      expect(await market1.totalLiquidity()).to.equal("9999");
      expect(await market2.totalLiquidity()).to.closeTo("3382", "1");
      ...
      expect(await market3.totalLiquidity()).to.closeTo("6764", "1");
    });
    ...
    it("allows leverage rate decrement", async function () {
      ...
      expect(await index.totalLiquidity()).to.equal("10073");
      expect(await market1.allocatedCredit(index.address)).to.equal("9999");
      expect(await market2.allocatedCredit(index.address)).to.equal("5073");
      expect(await market3.allocatedCredit(index.address)).to.equal("5073");
      expect(await index.totalAllocatedCredit()).to.equal("20145");
      expect(await vault.valueAll()).to.equal("10075");
      expect(await vault.totalAttributions()).to.equal("10075");
      expect(await vault.underlyingValue(index.address)).to.equal("10073");
      expect(await market1.availableBalance()).to.equal("0");
      expect(await market2.availableBalance()).to.equal("5073");
      expect(await market3.availableBalance()).to.equal("5073");
      expect(await index.leverage()).to.equal("1999");
      expect(await index.withdrawable()).to.equal("0");
      expect(await vault.attributions(index.address)).to.equal("10073");
    });
  });
